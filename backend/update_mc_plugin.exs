...
# Set up paths to load JSON files from the Docker container
plugin_json_path = "/app/mc-plugin-json"
...
# Find the existing Marvel Champions plugin
existing_plugin = Repo.one(from p in Plugin, where: p.name == "Marvel Champions")

plugin_params = %{
  name: "Marvel Champions",
  version: if(existing_plugin, do: existing_plugin.version + 1, else: 1),
  game_def: game_def,
  card_db: card_db,
  num_favorites: 0,
  public: true,
